 ...
 */
class WooSEA_Elite_Get_Products {
	...
	public function woosea_create_xml_feed ( $products, $feed_config, $header ) {
		...
		if ($feed_config['taxonomy'] == 'google_shopping'){
			...
			if ( ($header == "true") AND ($feed_config['nr_products_processed'] == 0) ) {
				...
				$xml->channel->addChild('description', 'WooCommerce Product Feed ELITE - This product feed is created with the Product Feed ELITE for WooCommerce plugin from AdTribes.io. For all your support questions check out our FAQ on https://www.adtribes.io or e-mail to: support@adtribes.io ');
				$xml->asXML($file);	
			} else {
...
//					$xml->addAttribute('xmlns:atom', 'http://www.w3.org/2005/Atom');
	...
	public function woosea_get_products ( $project_config ) {
		...
		$versions = array (
			...
			"Plugin" => WOOCOMMERCESEA_ELITE_PLUGIN_VERSION
		);
		...
		$shipping_zones = $this->woosea_get_shipping_zones();
			...
			*/
			 ...
			 */
			if (($product->is_type('simple')) OR ($product->is_type('external')) OR ($product->is_type('bundle')) OR ($product->is_type('composite')) OR ($product_data['product_type'] == "variable") OR ($product->is_type('auction'))){
