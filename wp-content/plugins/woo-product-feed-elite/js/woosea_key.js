jQuery(document).ready(function($) {
 	...
 	jQuery("#deactivate_license").click(function(){
		...
		jQuery.ajax({
                	method: "POST",
                       	url: ajaxurl,
                       	data: { 'action': 'woosea_elite_deactivate_license' }
                })
                .done(function( data ) {
 	...
 	jQuery("#checklicense").click(function(){
		...
		jQuery.ajax({
    			url: 'https://www.adtribes.io/check/license.php?key=' + license_key + '&email=' + license_email + '&domain=' + root_domain + '&version=1.3.2',
			jsonp: 'callback',
    			dataType: 'jsonp',
			type: 'GET',
			success: function( licenseData ) {
				...
				jQuery.ajax({
                        		method: "POST",
                        		url: ajaxurl,
                        		data: { 'action': 'woosea_elite_register_license', 'notice': notice, 'message_type': message_type, 'license_email': license_email, 'license_key': license_key, 'license_valid': license_valid, 'license_created': license_created, 'message': message }
                		})
                		.done(function( data ) {
                        		data = JSON.parse( data );	
				})
